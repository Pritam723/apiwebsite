...
def realTimeSCADAData():
    destination_path = r"./scada/er_data1.txt"  # Save in the current folder
    print(os.getcwd())
    try:
        network_path = r"\\10.3.200.53\scada\Documents\\er_data1.txt"
        shutil.copy(network_path, destination_path)
        print("Copy done")
    ...
    try:
        ...
        timestamp = os.path.getmtime(destination_path)

        # Convert to datetime object
        dt = datetime.datetime.fromtimestamp(timestamp)

        df = pd.read_csv(destination_path, header=None, index_col=0)
